...
const ActiveDeliveriesSection: React.FC<Props> = ({ deliveries, onViewInfo, onBrowseMarket }) => {
  ...
  if (deliveries.length === 0) {
    return (
      <div className="py-24 bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-700 text-center flex flex-col items-center">
              ...
              <tr key={delivery._id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/50 transition-colors group">
                ...
                <td className="px-8 py-6">
                  <div className="flex items-center gap-3 flex-wrap">
                    <span className="text-sm font-semibold max-w-[160px] truncate text-slate-800 dark:text-slate-200">{delivery.pickupAddress}</span>
                    <FiArrowRight className="text-slate-300 dark:text-slate-600 shrink-0" />
                    <span className="text-sm font-semibold max-w-[160px] truncate text-slate-800 dark:text-slate-200">{delivery.deliveryAddress}</span>
                  </div>
                  {customer && (
                    <p className="text-xs text-blue-600 dark:text-blue-400 font-medium mt-1">Customer: {(customer as any).name}</p>
                  )}
                </td>
                <td className="px-8 py-6">
